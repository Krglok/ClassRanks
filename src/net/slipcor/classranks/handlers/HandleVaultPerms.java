...
/**
 * Vault permissions handler class This will be the default Handler for
 * permissions
 * 
 * @version v0.4.3
 * 
...
public class HandleVaultPerms extends CRHandler
{
	...
	public boolean setupPermissions()
	{
		...
		try
		{
			...
			// getPermNameByPlayer(Bukkit.getWorlds().get(0).getName(),
			// "slipcor");
		} catch (Exception e)
		{
			return false;
	...
	/*
	 * Add a user to a given class in the given world diese funktion ist
	...
	public void classAdd(String world, Player player, String cString)
	{
		...
		// player = PlayerManager.searchName(player); // auto-complete
		// playername
		...
		{
			ClassRanks.log("Exception Add Class " + cString + " or user "
					+ player + " not found in world " + world, Level.WARNING);
		}
	}
...
//			{
//				ClassRanks.log("ERROR ADD PermName " + cString + " or user "
//						+ player + " ", Level.SEVERE);
//
//			} else
//			{
//				ClassRanks.log("Vault Global PermName " + cString + ": user "
//						+ player, Level.INFO);
//				plugin.db.i("Added Class/Rank " + cString + " to player "
//						+ player);
//			}
//		} catch (Exception e)
//		{
//			ClassRanks.log("Exception Add Class Global " + cString
//					+ " or user " + player, Level.SEVERE);
//		}
//	}

	/*
	 * Add a user to a given rank in the given world if world = all then rank
	 * added without world stores player Rank also in the config
	 */
	@Override
	public void rankAdd(Player player, String rank, String world)
	{
		if (plugin.config.isDefaultrankallworlds())
		{
			System.out.println("CR rankAddGlobal");
		...
		try
		{
			...
			{
				plugin.db.i("Vault added rank " + rank + " to player "
						+ player + " in world "
						+ world);
				// String cString = ClazzList.getClassNameByPermName(rank);
				// plugin.getConfig().set("players." + player + "." +
				// cString, rank);
				// plugin.getConfig().set("worlds." +
				// player.getWorld().getName() + "." + player, rank);
				// plugin.saveConfig();
				plugin.db.i("Vault PermName " + rank + ": user " + player
						+ " set " + world);
		...
		{
			ClassRanks.log("Exception added rank " + rank + " or user "
					+ player + " not found in world "
					+ player.getWorld().getName(), Level.WARNING);
		}
	}

//	/**
//	 * add rank/group in all worlds world set to null
//	 */
//	@Override
//	public void rankAddGlobal(Player player, String rank)
//	{
//		ClassRanks.log("ADD PermName " + rank + " or user " + player + " ",
//				Level.INFO);
//		// player = PlayerManager.searchName(player); // auto-complete
//		// playername
...
//			{
//				ClassRanks.log("ERROR ADD PermName " + rank + " or user "
//						+ player + " ", Level.SEVERE);
//
//			} else
//			{
//				ClassRanks.log("Vault Global PermName " + rank + ": user "
//						+ player, Level.INFO);
//				plugin.db.i("Global added rank " + rank + " to player "
//						+ player);
//
//			}
//		} catch (Exception e)
//		{
//			ClassRanks.log("Exception ADD GLOBAL RANK " + rank + " or user "
//					+ player, Level.SEVERE);
...
//	}
	 ...
	 * 
	 * @see
	 * net.slipcor.classranks.handlers.CRHandler#getPermNameByPlayer(java.lang
	...
	public String getPermNameByPlayer(String world, Player player)
	{
		...
		} else
		{
			try
			{
				...
				// plugin.getConfig().getConfigurationSection("worlds." +
				// world);
				groups = permission.getPlayerGroups(world, player.getName());
			} catch (Exception e)
			{
				plugin.db
						.i("Exception GetPermByPlayer : GetConfigurationSection");
				return "";
			}
...
//	}
	...
	/**
	 * Remove a user from the Rank in world that is current if world = all then
	 * rank added without world Remove player Rank also in the config
	 */
	@Override
	public void rankRemove(Player player, String rank, String world)
	{
		// player = PlayerManager.searchName(player); // auto-complete
		// playername
		if (plugin.config.isDefaultrankallworlds())
		...
		try
		{
			if (permission.playerRemoveGroup(world,player, rank))
			{
				plugin.db.i("removed rank " + rank + " from player "
						+ player.getDisplayName() + " in world ");
			} else
			{
				plugin.db.i("NOT removed rank " + rank + " from "
						+ player.getDisplayName() + " in world ");
			}
		} catch (Exception e)
		{
			ClassRanks.log("Exception Remove Rank " + rank + " or user "
					+ player + " not found in world ", Level.WARNING);
		}
	}
...
//		} catch (Exception e)
//		{
//			ClassRanks.log("Exception Remove Group Global" + rank + " or user "
//					+ player.getDisplayName() + " not found ", Level.SEVERE);
//		}
//	}
	 ...
	 */
	@Override
	public void removeGroups(Player player, String world)
	{
		if (plugin.config.isDefaultrankallworlds())
		{
			System.out.println("CR rankAddGlobal");
		...
		if (list != null)
		{
			for (String sRank : list)
			{
				permission.playerRemoveGroup(world, player, sRank);
			}
		}
	}

	@Override
	public String[] getPlayerGroups(Player player, String world)
	{
		if (plugin.config.isDefaultrankallworlds())
		{
			System.out.println("CR rankAddGlobal");
