...
public class ClassAdminCommand implements CommandExecutor 
{
	...
	public void cmdNone(Player pPlayer)
	{
		...
		plugin.msg(pPlayer, ChatColor.YELLOW+"description: Administrate the Classes and Ranks");
		plugin.msg(pPlayer, ChatColor.YELLOW+"usage: only for Ops and Admin (classrank.admin)");
		plugin.msg(pPlayer, ChatColor.YELLOW+" ");
		plugin.msg(pPlayer, ChatColor.YELLOW+"/classadmin add class [classname] [permname] [displayname] [color] [item:0] [cost] [exp]");
		plugin.msg(pPlayer, ChatColor.YELLOW+"/classadmin add rank  [classname] [permname] [displayname] [color] [item:0] [cost] [exp]");
		plugin.msg(pPlayer, ChatColor.YELLOW+"/classadmin change class [classname] [newname]");
		plugin.msg(pPlayer, ChatColor.YELLOW+"/classadmin change rank  [classname] [rankname] [displayname] [color]");
		plugin.msg(pPlayer, ChatColor.YELLOW+"/classadmin remove class [classname]"+ChatColor.RED+"Be careful ");
		plugin.msg(pPlayer, ChatColor.YELLOW+"/classadmin remove rank [classname] [rankname]"+ChatColor.RED+"Be careful ");
		plugin.msg(pPlayer, ChatColor.YELLOW+" ");
		plugin.msg(pPlayer, ChatColor.YELLOW+"/classadmin list {classname} {rankname}");
			...
			for (Clazz clazz : plugin.clazzList().getClazzes().values()) 
			{
				if (clazz.clazzName.equalsIgnoreCase(args[1]) ) 
				{
					...
					for (Rank r : clazz.ranks.values()) 
					{
						...
						{
							plugin.msg(pPlayer, "Rank: " + r.getColor() + r.getDispName());
							plugin.msg(pPlayer, "Perm: " + r.getColor() + r.getPermName());
							plugin.msg(pPlayer, "Cost: " + r.getColor() + r.getCost());
							plugin.msg(pPlayer, "Exp  : " + r.getColor() + r.getExp());
							plugin.msg(pPlayer, "Items : " );
							if (r.getItems() != null)
							{
								for (String sItem : r.getItems())
								{
									plugin.msg(pPlayer, "=> " + r.getColor() + sItem);
								}
								for (ItemStack item : r.getItemstacks())
								{
									if (item != null)
									{
										plugin.msg(pPlayer, "=> " + r.getColor() + item.getType()+":"+item.getAmount());
									}
								}
							}
							plugin.msg(pPlayer, " " );
	...
	private boolean cmdConfig(Player pPlayer, String[] args)
	{
		...
		pPlayer.sendMessage(ChatColor.YELLOW+"Debug  :  "+ChatColor.RED+plugin.config.isDebug());
		pPlayer.sendMessage(ChatColor.YELLOW+"Update :  "+ChatColor.GREEN+plugin.config.isUpdateCheck());
		pPlayer.sendMessage(ChatColor.WHITE+"AutoUpdate : "+ChatColor.GREEN+plugin.config.isAutoUpdate());
		...
		pPlayer.sendMessage(ChatColor.YELLOW+"TrackRanks:  "+ChatColor.GREEN+plugin.config.isTrackRanks());
		pPlayer.sendMessage(ChatColor.GREEN+"Default Cost  :  ");
		for(int i=0; i < plugin.config.getMoneyCost().length; i++)
		{
			pPlayer.sendMessage(ChatColor.YELLOW+"Rank "+(i+1)+": "+ChatColor.GREEN+String.valueOf(plugin.config.getMoneyCost()[i]));
			
		}
		pPlayer.sendMessage(ChatColor.GREEN+"Default Exp  :  ");
		for(int i=0; i < plugin.config.getExpCost().length; i++)
		{
			pPlayer.sendMessage(ChatColor.YELLOW+"Rank "+(i+1)+": "+ChatColor.GREEN+String.valueOf(plugin.config.getExpCost()[i]));
		...
		for(int i=0; i < plugin.config.getItemStacks().length; i++)
		{
			...
			{
				pPlayer.sendMessage(ChatColor.YELLOW+"   => "+ChatColor.GREEN+plugin.config.getItemStacks()[i][j].getType()+" : "+plugin.config.getItemStacks()[i][j].getAmount());
			}
	...
	private boolean cmdDebug (Player player, String[] args)
	{
		...
		player.sendMessage(ChatColor.YELLOW+"----------"+"Ver: " + ChatColor.GREEN+plugin.getDescription().getVersion());
		player.sendMessage(ChatColor.YELLOW+"Debug  :  "+ChatColor.RED+plugin.config.isDebug());
		return true;
	}
			 ...
			 */
				...
				if (args.length < 6) 
				{
					plugin.msg(
							pPlayer,
							"Wrong number of arguments ("
									+ String.valueOf(args.length) + ") should be minimum 6 !");
					cmdNone(pPlayer);
					return true;
				}
				ItemStack[] items;
